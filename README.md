...
- 초기 구글 로그인 시 발급된 토큰을 쿠키에 저장하였습니다. 이 토큰은 서버에 요청을 보낼 때마다 사용자 검증을 위해 함께 보내주었습니다. 문제는 로그인 토큰이 만료되었을 시에 클라이언트에서 확인하는 절차를 갖지 않았고, 만료 되었을 시에 화면 상에 변화를 주는 등의 사용자 경험에 이로운 조치를 취하지 않았다는 점입니다. 뒤늦게 jwt-decode를 통해 클라이언트에서 사용자 토큰 만료기간을 확인하도록 하여, 토큰이 만료되었을 시에 로그인 페이지로 이동하게끔 수정하였습니다. 이 방법이 전보다는 나아진 사용자 경험을 보장하지만, 토큰이 만료되는 1시간이라는 시간 동안 로스터 구성 페이지에 남아있게 되는 경우에 사용자가 입력한 데이터가 손실될 수 있다는 한계가 있었습니다. 코드에 반영하지는 못하였지만, 이를 해결하기위해 초기에 리프레쉬 토큰도 함께 발급해주어 토큰이 만료되었을 때 토큰 연장 요청을 보내는 방법으로 개선할 수 있다는 것을 배웠습니다. 또한 로그인과 사용자 경험의 연관성에 대해 고민하고 느낄 수 있었습니다.
